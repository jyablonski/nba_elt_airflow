...
# reminder that this is absolutely fucking retarded lmfao
# Copied from https://docs.astronomer.io/learn/scheduling-in-airflow
class UnevenIntervalsTimetable(Timetable):
    def infer_manual_data_interval(self, run_after: DateTime) -> DataInterval:
        ...
        if run_after >= run_after.set(hour=6, minute=0) and run_after <= run_after.set(
            hour=16, minute=30
        ):
            start = (
                (run_after - delta)
                .set(hour=16, minute=30, second=0)
                .replace(tzinfo=UTC)
            )
    ...
    def next_dagrun_info(
        ...
        *,
        ...
        if (
            ...
            if (
                last_start.hour == 6
            ):  # If previous period started at 6:00, next period will start at 16:30 and end at 6:00 following day
                ...
                next_start = (
                    (last_start + delta).set(hour=6, minute=0).replace(tzinfo=UTC)
                )
                next_end = (last_start + delta).replace(tzinfo=UTC)
